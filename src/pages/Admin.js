...
const STAT_GROUPS = [
  {
    ...
    color: '#2d9e62',
    stats: [
      { key: 'forhånd', label: 'Forhånd' },
      { key: 'baghånd', label: 'Baghånd' },
    ],
  },
  {
    label: 'Overheads',
  ...
  },
  {
    ...
    color: '#185fa5',
    stats: [
      { key: 'volley_forhånd', label: 'Forhånd volley' },
      { key: 'volley_baghånd', label: 'Baghånd volley' },
      { key: 'plano', label: 'Plano' },
    ],
  },
  {
    label: 'Omstillingsslag',
    color: '#854f0b',
    stats: [
  ...
  },
  {
    ...
    color: '#534ab7',
    stats: [
      { key: 'glasspil', label: 'Spil efter glas' },
      { key: 'spilforstaelse', label: 'Spilforståelse' },
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
        <label style={{ fontSize: 13, color: 'var(--color-text-primary)', fontWeight: 400 }}>{label}</label>
        <div style={{
          background: '#0d1f2d', color: '#f5e642', fontWeight: 700,
          fontSize: 15, borderRadius: 6, padding: '2px 9px', minWidth: 38, textAlign: 'center'
        }}>{value}</div>
      </div>
      <div style={{ position: 'relative', height: 8 }}>
        <div style={{
          position: 'absolute', inset: '3px 0', background: 'var(--color-border-tertiary)',
...
export default function Admin() {
  ...
  const ADMIN_PIN = '1234' // Skift dette til jeres eget pin
  ...
  const updateStat = (key, val) => {
    setLocalStats(prev => ({ ...prev, [key]: val }))
  }
  ...
  const offScore = avg(
    localStats.bandeja || 50, localStats.vibora || 50, localStats.rulo || 50,
    localStats.gancho || 50, localStats.smash || 50,
    localStats['volley_forhånd'] || 50, localStats['volley_baghånd'] || 50, localStats.plano || 50
  )
  const defScore = avg(
    localStats['forhånd'] || 50, localStats['baghånd'] || 50,
    localStats.chiquita || 50, localStats.lob || 50, localStats.glasspil || 50
  )
  ...
  if (!unlocked) return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginBottom: 20 }}>Indtast PIN for at redigere spillerratings</div>
  ...
  if (!selected) return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <span style={{ fontSize: 16, fontWeight: 500 }}>Admin — Ratings</span>
        <button onClick={() => setUnlocked(false)} style={{ fontSize: 12, color: 'var(--color-text-tertiary)', background: 'none', border: 'none', cursor: 'pointer' }}>Lås</button>
      </div>
      <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginBottom: 14 }}>Vælg en spiller for at opdatere ratings</div>
          ...
          return (
            <div key={p.id}
              ...
              style={{
                ...
                gap: 12, cursor: 'pointer', transition: 'border-color .15s'
              }}
  ...
  return (
    <>
      ...
      {/* Player header */}
      <div style={{
        background: 'linear-gradient(145deg, #1a2e4a 0%, #0d1f2d 100%)',
        borderRadius: 14, padding: 16, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 14, color: '#fff'
      }}>
        <div style={{
          width: 52, height: 52, borderRadius: '50%', background: selected.color,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 18, fontWeight: 700, border: '2px solid rgba(255,255,255,0.2)'
      ...
      {STAT_GROUPS.map(group => (
        <div key={group.label} style={{
          ...
          <div style={{
            ...
            color: group.color, marginBottom: 12, paddingBottom: 8,
      ...
      <button
        ...
        onClick={save}
        disabled={saving}
        style={{ marginTop: 4, marginBottom: 24, opacity: saving ? 0.7 : 1 }}
      >
