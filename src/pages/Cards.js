...
const STAT_FILTERS = [
  { group: 'Grundslag', items: [{ key: 'forhnd', label: 'Forhand' }, { key: 'baghnd', label: 'Baghand' }]},
  ...
  { group: 'Volleys', items: [{ key: 'volley_forhnd', label: 'Forhand volley' }, { key: 'volley_baghnd', label: 'Baghand volley' }, { key: 'plano', label: 'Plano' }]},
  { group: 'Omstilling', items: [{ key: 'chiquita', label: 'Chiquita' }, { key: 'lob', label: 'Lob' }]},
  { group: 'Generelt', items: [{ key: 'glasspil', label: 'Glasspil' }, { key: 'spilforstaelse', label: 'Spilforstaelse' }, { key: 'bevaegelse', label: 'Bevaegelse' }, { key: 'kommunikation', label: 'Kommunikation' }]},
]
      ...
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <button onClick={() => setShowFilters(f => !f)} style={{ fontSize: 12, padding: '5px 12px', borderRadius: 8, border: '0.5px solid var(--color-border-secondary)', background: showFilters ? '#0d1f2d' : 'none', color: showFilters ? '#fff' : 'var(--color-text-secondary)', cursor: 'pointer' }}>
            {showFilters ? 'Skjul' : 'Filtre'}{hasActiveFilters ? ' *' : ''}
          </button>
      ...
      {showFilters && (
        <div style={{ background: 'var(--color-background-primary)', border: '0.5px solid var(--color-border-tertiary)', borderRadius: 12, padding: 16, marginBottom: 14 }}>
              ...
              {POSITIONS.map(pos => (
                <button key={pos} onClick={() => setPosFilter(pos)}
                  style={{ padding: '5px 14px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 12,
                    background: posFilter === pos ? '#0d1f2d' : 'var(--color-background-secondary)',
                    color: posFilter === pos ? '#fff' : 'var(--color-text-secondary)' }}>
                  {pos === 'HO' ? 'Hojre' : pos === 'VN' ? 'Venstre' : pos}
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>
              {FILTER_PRESETS.map(preset => (
                <button key={preset.id} onClick={() => setSortKey(preset.key)}
                  style={{ padding: '5px 12px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 12,
                    background: sortKey === preset.key ? '#1a7a4a' : 'var(--color-background-secondary)',
                    color: sortKey === preset.key ? '#fff' : 'var(--color-text-secondary)' }}>
                  {preset.label}
                </button>
              ))}
            </div>
            {STAT_FILTERS.map(group => (
              <div key={group.group} style={{ marginBottom: 8 }}>
                <div style={{ fontSize: 10, color: 'var(--color-text-tertiary)', marginBottom: 4 }}>{group.group}</div>
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                  {group.items.map(item => (
                    <button key={item.key} onClick={() => setSortKey(item.key)}
                      style={{ padding: '4px 10px', borderRadius: 16, border: '0.5px solid var(--color-border-tertiary)', cursor: 'pointer', fontSize: 11,
                        background: sortKey === item.key ? '#185fa5' : 'none',
                        color: sortKey === item.key ? '#fff' : 'var(--color-text-secondary)' }}>
                      {item.label}
                    </button>
                  ))}
                </div>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[['desc', 'Hojest forst'], ['asc', 'Lavest forst']].map(([d, label]) => (
                <button key={d} onClick={() => setSortDir(d)}
                  style={{ padding: '5px 14px', borderRadius: 20, border: 'none', cursor: 'pointer', fontSize: 12,
                    background: sortDir === d ? '#0d1f2d' : 'var(--color-background-secondary)',
                    color: sortDir === d ? '#fff' : 'var(--color-text-secondary)' }}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input type="range" min={0} max={90} step={5} value={minRating}
